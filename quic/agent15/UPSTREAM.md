...
| `extension/KeyShareExtension.java` | a server key_share entry keeps its key exchange value raw, so a `ClientHelloSpec` can own a group agent15 has no key exchange for. The bytes are copied before `rawToEncodedXDHPublicKey`, which reverses the array it is given in place - upstream never notices, having no use for them afterwards |
...
| `engine/impl/TlsClientEngineImpl.java` (ALPS and early data) | refuses the one combination no capture covers: a server that echoed "application_settings" in an EncryptedExtensions that also accepted early data. BoringSSL treats ALPS as settled on both sides when early data is accepted - its server does not echo the extension and its client sends no EncryptedExtensions of its own - so a server that echoes it is either asking for the client's message or repeating what was agreed, and those want opposite behaviour. Verified not to cost real traffic: google.com negotiates ALPS, accepts this client's 0-RTT and does not echo it |
| `handshake/ClientHello.java` (spec constructor length) | refuses a message it cannot describe. The handshake length is a uint24 and this writes the low two bytes of it, which is right for every ClientHello there is and silently wrong for one of 65536 bytes; the other constructor overflows its 3000 byte buffer first, this one sizes its buffer to fit |
| `extension/KeyShareExtension.java` (unknown group) | a ServerHello whose key_share names a group this implementation has no codepoint for is refused, naming the codepoint. The entry was dropped and a ServerHello's key_share has exactly one, which the engine reads as `get(0)`, so the server picking an unknown group would have surfaced as "Index 0 out of bounds for length 0" somewhere else entirely. A ClientHello is unchanged: a client may offer GREASE groups and a server must skip them |
