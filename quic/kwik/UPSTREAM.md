...
| `recovery/RecoveryManager.java` (probe contents) | a PTO that sends two packets fills them from the two oldest unacknowledged packets rather than sending the first one's frames twice. RFC 9002 section 6.2.4 allows either - the second datagram is there "to avoid an expensive consecutive PTO expiration due to a single lost datagram", and "Implementations MAY use alternative strategies for determining the content of probe packets" - and the alternative is worth it for a browser's first flight, which is two Initial packets: while nothing is acknowledged, repeating only the first leaves the second unsent however many probes go out. With one packet outstanding it repeats that one, which is the redundancy the section describes |
| `QuicClientConnection.java` (onNewSessionTicket, onNewToken) | a caller can be told as each session ticket and address validation token arrives, instead of asking afterwards. `getNewSessionTickets()` and `getNewTokens()` answer with what has arrived so far, which is enough only for a caller that asks once at the end and gets that far: a connection that is kept has them arriving throughout and none of them reachable by anything else until somebody asks, and one that is dropped rather than closed is never asked. It does not conjure what has not arrived - a server that sends its ticket after the response has sent nothing by the time a caller that closes at once has closed |
